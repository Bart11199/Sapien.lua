--[[
    SAPIEN V1 Enhanced Cheat Script
    Funciones:
      ...
      • PERFECTSHOT: Auto-aim activado por un solo botón.
          ...
          – Mientras esté activo, si algún enemigo (cualquier jugador menos tú)
            tiene su "Head" dentro de ese círculo (umbral configurable), la cámara se ajusta automáticamente (lock-on).
      ...
      • AntiDetect: Botón toggle (funcionalidad dummy, ya que un anti-detect real es muy complejo).
...
------------------------------------------------
-- Menú Principal Arrastrable (más chico para mobile)
------------------------------------------------
local MenuFrame = Instance.new("Frame")
MenuFrame.Name = "MenuFrame"
MenuFrame.Size = UDim2.new(0, 280, 0, 450)  -- Menú más pequeño
...
-- Crear un AimCircle (pequeño círculo en el centro de la pantalla) para auto-aim
...
RunService.Heartbeat:Connect(function()
    if perfectShotEnabled and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        ...
        if bestTarget then
            -- Lock on: ajustar la cámara para mirar al target (usando tween según la potencia)
...
-- Loop para forzar el ammo a un valor alto (esto es muy dependiente del juego)
spawn(function()
    while wait(0.1) do
...
-- Función dummy para buscar y modificar "Damage" en el character o tools
spawn(function()
    while wait(0.5) do
...
AntiDetectButton.MouseButton1Click:Connect(function()
    ...
    if antiDetectEnabled then
        ...
        -- Aquí se podrían agregar técnicas adicionales para ofuscar el script.
    else
        AntiDetectButton.Text = "AntiDetect: OFF"
        AntiDetectButton.BackgroundColor3 = Color3.new(0.8,0,0)
    end
